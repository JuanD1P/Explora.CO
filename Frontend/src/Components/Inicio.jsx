...
const ImgSmart = ({ src, alt, className, w = 1200, h = 800, loading = "lazy", fetchpriority }) => {
...
const Carousel = ({ children, className, ariaLabel, dots = true, step = 0.9, showArrows = true }) => {
  const trackRef = useRef(null);
  const { prev, next } = useScrollButtons(trackRef, step);

  // paginación simple: número de slides
...
/* ===================== Lista de Departamentos (SIN coverflow) ===================== */
const DepsGridScroll = ({ deps, depImgs }) => {
  const trackRef = useRef(null);
          ...
          return (
            <div className="carousel__slide dep-card-wrap" key={d}>
              <a href={`/departamentos/${slug}`} className="dep-card" aria-label={`Ver destinos en ${d}`}>
                {/* borde/shine/skeleton eliminados */}
                <div className="dep-card__media">
                  <ImgSmart src={src} alt={d} className="dep-card__img" w={640} h={420} />
...
const Inicio = () => {
  ...
  /* Hero manual (SIN auto-avance ni parallax) */
  const heroTrackRef = useRef(null);

  return (
    <main className="ini-root" role="main">
      {/* BG decorativo — estático */}
      ...
      <section className="ini-intro">
        ...
        <div className="ini-hero card-3d">
          ...
          <div className="hero carousel ini-hero-carousel">
            ...
            {/* Bullets (pasivas, sin animaciones) */}
            <div className="carousel__dots" role="tablist" aria-label="Paginación">
      ...
      <section className="ini-section">
        ...
        <Carousel className="ini-carousel glass" ariaLabel="Carrusel de categorías" step={0.8}>
          {categorias.map((c, idx) => (
            <a key={c.id} href={c.href} className="ini-card hover-raise carousel__slide" aria-label={c.titulo}>
              <div className="ini-thumb">
                <ImgSmart src={catImgs[idx]} alt={c.titulo} w={600} h={400} />
              </div>
              <span>{c.titulo}</span>
            </a>
          ))}
      ...
      <section className="ini-banner card-3d">
        <ImgSmart src="/ImagenesP/Banners/elige-destino.jpg" alt="Banner informativo" w={1920} h={480} />
      </section>
      ...
      <section className="ini-section" aria-labelledby="deps-title">
        ...
        <DepsGridScroll deps={departamentos} depImgs={depImgs} />
      </section>
    </main>
  );
};
